 ...
 */
define(['N/search', 'N/ui/serverWidget', 'N/runtime'], (SEARCH, serverWidget, RUNTIME) => {
  function beforeLoad(context) {
    ...
    if (results.length > 0) {
      ...
      htmlStr += `
        ...
        ${indexes.map(
          ...
          ${indexes
            ...
            .join('\n')}
          ${indexes
            .map((res, ind) => `${res} iinput[type="radio"]:checked ~ span {  color: #0070d2; }`)
            .join('\n')}
      ...
      groups.forEach((group) => {
        ...
        htmlStr += `<div style="flex:2; margin:10px; border:1px solid #eee; border-radius:8px; padding:16px; ">`;
        ...
        groupData.fields.forEach((result, ind) => {
          ...
          const htmlText = result.getValue('custrecord_mhi_ecl_cust_html_text') || '';
  ...
  const getFieldConfigs = (recType) => {
    const recTypeId = RUNTIME.getCurrentScript().getParameter('custscript_mhi_ecl_crt_rec_type');
    ...
    htmlConfigSearch.run().each((result) => {
      const group = result.getText('custrecord_mhi_ecl_cust_html_group') || 'MISC';
      if (group) {
        if (!groupedRec[group]) {
          groupedRec[group] = {
